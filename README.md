...
- `engine/assist.c` — the assist-mode C exports (level/kill/secret readers, autosave, the twin-stick touch controls) that `web/shell.html` calls into. Two small patches on top of doomgeneric itself: one line in `g_game.c` to add the touch sticks' strafe/forward/turn straight into the same locals keyboard and mouse input already feed, and one line in `doomgeneric_emscripten.c` to point saved games at the browser's persistent storage. Both are marked `--- doom-assist patch ---` inline.

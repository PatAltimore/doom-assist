...
{
    // doom-assist: this vendored net_client.c (chocolate-doom 2.3.0) calls
    // M_BindStringVariable, a typed wrapper this project's own (older)
    // m_config.c never gained -- but the "player_name" default itself is
    // already registered there (CONFIG_VARIABLE_STRING(player_name)), so
    // the untyped M_BindVariable it already has does the same job.

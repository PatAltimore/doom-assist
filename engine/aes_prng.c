...
#include <stdint.h>
// doom-assist: upstream chocolate-doom's own i_swap.h wraps SDL_endian.h's
// SDL_SwapLE32 directly, but doomgeneric's i_swap.h (vendored in this repo)
// was rewritten to avoid an SDL dependency in the core engine, using
// __BYTE_ORDER__ instead -- it doesn't define SDL_SwapLE32 at all. This
// file is a self-contained PRNG utility with no real portability reason to
// avoid SDL, and this build already links SDL2 (-sUSE_SDL=2, see
// build-emscripten.sh) for i_video.c/i_sound.c, so pulling the real
// definition in directly here is simpler than reimplementing a swap macro.
